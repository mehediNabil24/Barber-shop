...
const HairCutDetails: React.FC<SalonCardProps> = ({
  ...
  const [currentMainImage, setCurrentMainImage] = useState(mainImage); // State for the main image
  ...
  const handleThumbnailClick = (image: string, index: number) => {
    setCurrentMainImage(image); // Update the main image to the clicked thumbnail
  };
      ...
      <div>
        <div className="flex flex-col gap-10">
          ...
          <div className="relative md:w-full w-[300px] h-[500px] rounded-lg overflow-hidden">
            <Image
              src={currentMainImage} // Use the dynamic main image state
              alt={title}
              fill
        ...
        <div className="relative flex items-center">
          ...
          {/* Gallery Images */}
          <div className="flex gap-2 overflow-hidden w-full mt-4">
            {galleryImages.slice(currentIndex, currentIndex + imagesPerView).map((image, index) => (
              <div
                key={index}
                className={`relative w-[105px] h-[105px] rounded-lg overflow-hidden flex-shrink-0 cursor-pointer border-2 ${
                  image === currentMainImage ? 'border-blue-500' : 'border-transparent'
                }`}
                onClick={() => handleThumbnailClick(image, index + currentIndex)}
              >
                <Image
                  src={image}
